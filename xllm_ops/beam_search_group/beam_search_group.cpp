...
__aicore__ inline void BeamSearchGroup<TokenIdType, LogProbType>::Process() {
  for (int32_t request_idx = 0; request_idx < this->request_num; request_idx++) {
    if (request_idx % 24 == block_idx) {
      ...
      AscendC::Duplicate<LogProbType>(prefix_top_probs, static_cast<LogProbType>(-1.0f / 0.0f),this->align_beam_width1);
      LocalTensor<int32_t> prefix_top_index = prefix_index_buf.GetWithOffset<int32_t>(this->align_beam_width1, 0);
      AscendC::Duplicate<int32_t>(prefix_top_index, static_cast<int32_t>(0),this->align_beam_width1);
...
__aicore__ inline void ProcessSequence::Init(GM_ADDR sequence, 
  ...
  pipe.InitBuffer(top_tokens_buf, this->top_k * this->top_k * sizeof(int32_t));
}
