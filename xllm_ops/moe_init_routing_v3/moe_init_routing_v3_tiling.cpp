...
{
    OPS_CHECK(activeExpertRangeListPtr_->GetSize() != ATTR_EXPERT_RANGE_DIM,
                OPS_LOG_E(context_, "The dim number of expert_range should be %ld.", ATTR_EXPERT_RANGE_DIM),
    ...
    OPS_CHECK(activeNum_ < ACTIVE_NUM_MIN_VALUE,
                OPS_LOG_E(context_, "active_num should be greater than or equal to -1"), return ge::GRAPH_FAILED);
    ...
    OPS_CHECK(dropPadMode_ != DROP_LESS, OPS_LOG_E(context_, "drop_pad_mode currently support %ld", DROP_LESS),
                return ge::GRAPH_FAILED);

    expertTokensNumType_ = *expertTokensNumTypePtr_;
    moeInitRoutingV3TilingData.set_expertTokensNumType(expertTokensNumType_);
    OPS_CHECK((expertTokensNumType_ != COUNT) && (expertTokensNumType_ != KEY_VALUE),
    ...
    OPS_CHECK(!expertTokensNumFlag_, OPS_LOG_E(context_, "expert_tokens_num_flag currently support True"),
                return ge::GRAPH_FAILED);
    ...
    OPS_CHECK(quantMode_ != UN_QUANT && quantMode_ != DYNAMIC_QUANT,
                OPS_LOG_E(context_, "quant_mode currently support %ld, %ld", UN_QUANT, DYNAMIC_QUANT),
                return ge::GRAPH_FAILED);
...
ge::graphStatus MoeInitRountingV3TilingBase::CheckInputShape()
{
    ...
    // OPS_LOG_I(context_, "input x shape: %s ", ToString(xShape).c_str());
    const gert::Shape expertIdxShape = expertIdxShapePtr_->GetStorageShape();
    // OPS_LOG_I(context_, "input expert_idx shape: %s.", ToString(expertIdxShape).c_str());
    ...
    if (activeNum_ != ACTIVE_NUM_MIN_VALUE) {	
        OPS_CHECK(activeNum_ != totalLength_,	
                    OPS_LOG_E(context_, "active_num currently should equal to %ld(bs*k)", totalLength_),	
                    return ge::GRAPH_FAILED);	
    }
    if (activeNum_ == 0) {
    ...
    if (quantMode_ == UN_QUANT && scaleShapePtr_ != nullptr) {
        ...
        // OPS_LOG_I(context_, "input scale shape: %s", ToString(scaleShape).c_str());
        OPS_CHECK(scaleShape.GetDimNum() != 1, OPS_LOG_E(context_, "The dim number of scale should be 1. "),
                    return ge::GRAPH_FAILED);
        OPS_CHECK(scaleShape.GetDim(0) != n_, OPS_LOG_E(context_, "The first dim of scale should be %ld", n_),
                    return ge::GRAPH_FAILED);
    }
    ...
    if (quantMode_ == DYNAMIC_QUANT && scaleShapePtr_ != nullptr) {
        auto scaleShape = scaleShapePtr_->GetStorageShape();
        // OPS_LOG_I(context_, "input scale shape: %s", ToString(scaleShape).c_str());
        OPS_CHECK(scaleShape.GetDimNum() != 2, OPS_LOG_E(context_, "The dim number of scale should be 2. "),
                    return ge::GRAPH_FAILED);
        OPS_CHECK(scaleShape.GetDim(0) != (expertEnd_ - expertStart_),
                    OPS_LOG_E(context_, "The first dim of scale should be %ld", (expertEnd_ - expertStart_)),
                    return ge::GRAPH_FAILED);
        OPS_CHECK(scaleShape.GetDim(1) != cols_, OPS_LOG_E(context_, "The second dim of scale should be %ld", cols_),
                    return ge::GRAPH_FAILED);
...
ge::graphStatus MoeInitRountingV3TilingBase::CheckOutShape()
{
    ...
    // OPS_LOG_I(context_, "expanded_x shape: %s.", ToString(expandedXShape).c_str());
    const gert::Shape expandedRowIdxShape = context_->GetOutputShape(1)->GetStorageShape();
    // OPS_LOG_I(context_, "expanded_row_idx shape: %s.", ToString(expandedRowIdxShape).c_str());
    const gert::Shape expertTokensCountOrCumsumShape = context_->GetOutputShape(2)->GetStorageShape();
    // OPS_LOG_I(context_, "expert_tokens_count_or_cumsum shape: %s.",
    //         ToString(expertTokensCountOrCumsumShape).c_str());

    OPS_CHECK(expandedXShape.GetDimNum() != DIM_TWO, OPS_LOG_E(context_, "The dim number of expanded_x should be 2."),
                return ge::GRAPH_FAILED);
    ...
    OPS_CHECK(expandedXShape.GetDim(0) != totalLength_,
                OPS_LOG_E(context_, "The first dim of expanded_x should be %ld.", totalLength_), return ge::GRAPH_FAILED);
    OPS_CHECK(expandedXShape.GetDim(1) != cols_,
                OPS_LOG_E(context_, "The second dim of expanded_x should be %ld.", cols_), return ge::GRAPH_FAILED);
    OPS_CHECK(
...
ge::graphStatus MoeInitRountingV3TilingBase::GetShapeAttrsInfo()
{
    ...
    OPS_LOG_E_IF_NULL(context_, xShapePtr_, return ge::GRAPH_FAILED);

    expertIdxShapePtr_ = context_->GetInputShape(INPUT_EXPERT_IDX_INDEX);
    OPS_LOG_E_IF_NULL(context_, expertIdxShapePtr_, return ge::GRAPH_FAILED);
...
{   
    ...
    OPS_LOG_I(context_, "isFullload: %ld, gatherFirstFullload: %ld, epFullload: %ld", isFullloadInt, gatherFirstFullload_, epFullload_);
}
...
bool MoeInitRountingV3TilingBase::IsFullLoad() {
    ...
    if (quantMode_ == -1) {
        ...
        if (epFullload_) {
            quantSpace = CeilDiv(cols_* STATIC_QUANT_FULLLOAD_COLS_BUFFER, ONE_BLOCK_BYTE) * ONE_BLOCK_BYTE;
        } else {
            quantSpace = CeilDiv(cols_* STATIC_QUANT_FULLLOAD_COLS_BUFFER, ONE_BLOCK_BYTE) * ONE_BLOCK_BYTE * perCoreTokens;
        }
        remainUb -= (gatherSpace + quantSpace);
    } else {
        int64_t quantSpace = CeilDiv(cols_ * DYNAMIC_QUANT_FULLLOAD_COLS_BUFFER, ONE_BLOCK_BYTE) * ONE_BLOCK_BYTE;
