                                                         ...
                                                         int64_t &experTokenNumType)
{
    ...
    if (experTokenNumType < ExpertTokenNumType::CUMSUM || experTokenNumType > ExpertTokenNumType::KEY_VALUE) {
        OPS_LOG_E(context, "The expert_token_num_type should be %d, %d or %d. But it is %ld.",
                                                ...
                                                int64_t &quantMode)
{
    ...
    if (nullptr == quantModePtr) {
        OPS_LOG_E(context, "The quant_mode should be %d, %d or %d. But it is none.", QuantMode::NON_QUANT,
    ...
    if (quantMode < QuantMode::NON_QUANT || quantMode > QuantMode::DYNAMIC_QUANT) {
        OPS_LOG_E(context, "The quant_mode should be %d, %d or %d. But it is %ld.", QuantMode::NON_QUANT,
                                            ...
                                            const int64_t expertEnd, const int64_t quantMode)
{
    ...
    OPS_CHECK((nullptr == scaleShape && QuantMode::STATIC_QUANT == quantMode),
    ...
    OPS_CHECK((nullptr == scaleShape && (QuantMode::NON_QUANT == quantMode || QuantMode::DYNAMIC_QUANT == quantMode)),
                OPS_LOG_I(context, "When quant_mode is NON_QUANT or DYNAMIC_QUANT, scale can be none."),
                return ge::GRAPH_SUCCESS);
    OPS_CHECK((nullptr == scaleShape),
                OPS_LOG_E(context, "The scale cannot be none when quant_mode is %ld.", quantMode),
                return ge::GRAPH_FAILED);
    ...
    OPS_CHECK(QuantMode::NON_QUANT == quantMode &&
                    (scaleShape->GetDimNum() != DIM_ONE || !isSameDim(scaleShape->GetDim(0), xShape->GetDim(0))),
                OPS_LOG_E(context, "The shape of scale should be (%ld), current shape is (%s).", xShape->GetDim(0),
                        ToString(*scaleShape).c_str()),
                return ge::GRAPH_FAILED);
    ...
    OPS_CHECK(QuantMode::STATIC_QUANT == quantMode &&
                    (scaleShape->GetDimNum() == DIM_ONE && !isSameDim(scaleShape->GetDim(0), activeExpertRange)),
                OPS_LOG_E(context, "When quant_mode=%ld, the shape of scale should be (%ld,), current shape is (%s).",
                        quantMode, activeExpertRange, ToString(*scaleShape).c_str()),
                return ge::GRAPH_FAILED);
    OPS_CHECK(QuantMode::STATIC_QUANT == quantMode &&
                    (scaleShape->GetDimNum() == DIM_TWO && !isSameDim(scaleShape->GetDim(0), activeExpertRange)),
                OPS_LOG_E(context, "When quant_mode=%ld, the scale_dim_0 should be %ld, but its shape is (%s).",
                        quantMode, activeExpertRange, ToString(*scaleShape).c_str()),
                return ge::GRAPH_FAILED);

    // When quant_mode is DYNAMIC_QUANT and scale is not none, the scale shape should be (end-start, h).
    OPS_CHECK(QuantMode::DYNAMIC_QUANT == quantMode &&
                    (scaleShape->GetDimNum() == DIM_ONE && !isSameDim(scaleShape->GetDim(0), activeExpertRange)),
                OPS_LOG_E(context, "When quant_mode=%ld, the scale shape should be (%ld,), current shape is (%s).",
                        quantMode, activeExpertRange, ToString(*scaleShape).c_str()),
                return ge::GRAPH_FAILED);
    OPS_CHECK(QuantMode::DYNAMIC_QUANT == quantMode &&
                    (scaleShape->GetDimNum() == DIM_TWO && (!isSameDim(scaleShape->GetDim(0), activeExpertRange) ||
                                                            !isSameDim(scaleShape->GetDim(1), xShape->GetDim(1)))),
                OPS_LOG_E(context, "When quant_mode=%ld, the scale shape should be (%ld, %ld), but its shape is (%s).",
                        quantMode, activeExpertRange, xShape->GetDim(1), ToString(*scaleShape).c_str()),
                return ge::GRAPH_FAILED);
                                             ...
                                             const int64_t expertEnd, const int64_t quantMode)
{
    ...
    if (QuantMode::STATIC_QUANT == quantMode) {
        if (offsetShape->GetDimNum() == DIM_ONE && !isSameDim(offsetShape->GetDim(0), activeExpertRange)) {
            OPS_LOG_E(context, "The shape of offset should be (%ld,), current shape is (%s).",
                    activeExpertRange, ToString(*offsetShape).c_str());
            return ge::GRAPH_FAILED;
        }

        if (offsetShape->GetDimNum() == DIM_TWO && !isSameDim(offsetShape->GetDim(0), activeExpertRange)) {
            OPS_LOG_E(context, "The offset_dim_0 should be %ld, but its shape is (%s).", activeExpertRange,
                    ToString(*offsetShape).c_str());
            return ge::GRAPH_FAILED;
        }
    }
                                       ...
                                       const int64_t expertEnd, const int64_t quantMode)
{
    ...
    if (xShape->GetDimNum() == DIM_ONE) {
        if (xShape->GetDim(0) != ge::UNKNOWN_DIM_NUM) {
            OPS_LOG_E(context, "The dynamic dim of x should be -2, current shape is %s.",
                    ToString(*xShape).c_str());
            return ge::GRAPH_FAILED;
        }
    } else if (xShape->GetDimNum() != DIM_TWO) {
        OPS_LOG_E(context, "The dim of x should be 2 or dynamic, current shape is %s.",
                ToString(*xShape).c_str());
        return ge::GRAPH_FAILED;
    }
    ...
    if (x_n < NEG_ONE || cols < NEG_ONE) {
        OPS_LOG_E(context, "Invalid x shape, shape is %s.", ToString(*xShape).c_str());
        return ge::GRAPH_FAILED;
    }
    ...
    if (expertIdxShape->GetDimNum() == DIM_ONE) {
        if (expertIdxShape->GetDim(0) != ge::UNKNOWN_DIM_NUM) {
            OPS_LOG_E(context, "The dynamic dim of expert_idx should be -2, current shape is %s.",
                    ToString(*expertIdxShape).c_str());
            return ge::GRAPH_FAILED;
        }
    } else if (expertIdxShape->GetDimNum() != DIM_TWO) {
        OPS_LOG_E(context, "The dim of expert_idx should be 2 or dynamic, current shape is %s.",
                ToString(*expertIdxShape).c_str());
        return ge::GRAPH_FAILED;
    }
    ...
    if (expert_idx_n < NEG_ONE || expert_idx_k < NEG_ONE) {
        OPS_LOG_E(context, "Invalid expert_idx shape, shape is %s.",
                ToString(*expertIdxShape).c_str());
        return ge::GRAPH_FAILED;
    }
                                      ...
                                      const int64_t expertEnd, const int64_t quantMode, const int64_t rowIdxType)
{
    ...
    OPS_LOG_D(context, "x shape is: %s.", ToString(*xShape).c_str());
    OPS_LOG_D(context, "expert_idx shape is: %s.", ToString(*expertIdxShape).c_str());

    // scale is optional and can be none.
    if (nullptr == scaleShape) {
        OPS_LOG_D(context, "scale_shape is: none.");
    } else {
        OPS_LOG_D(context, "scale_shape is: %s.", ToString(*scaleShape).c_str());
    }
    ...
    if (nullptr == offsetShape) {
        ...
        OPS_LOG_D(context, "offset_shape is: %s.", ToString(*offsetShape).c_str());
    }
...
{
    OPS_LOG_D(context, "expanded_x shape is: %s after infershape.",
            ToString(*expandedXShape).c_str());
    OPS_LOG_D(context, "expanded_row_idx shape is: %s after infershape.",
            ToString(*expandedRowIdxShape).c_str());
    OPS_LOG_D(context, "expert_token_cumsum_or_count shape is: %s after infershape.",
            ToString(*expertTokenCumsumOrCountShape).c_str());
    OPS_LOG_D(context, "expanded_scale shape is: %s after infershape.",
            ToString(*expandedScaleShape).c_str());
}
...
static ge::graphStatus InferShape4MoeInitRoutingV3(gert::InferShapeContext *context)
{
    ...
    OPS_LOG_E_IF_NULL(context, expertTokenCumsumOrCountShape, return ge::GRAPH_FAILED);
    gert::Shape *expandedScaleShape = context->GetOutputShape(MOE_INIT_ROUTING_V3_OUTPUT_EXPANDED_SCALE);
    OPS_LOG_E_IF_NULL(context, expandedScaleShape, return ge::GRAPH_FAILED);
...
{
    // OPS_LOG_D(context, "Begin to do MoeInitRoutingV3InferRange.");
    ...
    // OPS_LOG_E_IF_NULL(context, expanded_x, return ge::GRAPH_FAILED);
    auto expanded_row_idx = context->GetOutputShapeRange(MOE_INIT_ROUTING_V3_OUTPUT_EXPANDED_ROW_IDX);
    // OPS_LOG_E_IF_NULL(context, expanded_row_idx, return ge::GRAPH_FAILED);
    auto count = context->GetOutputShapeRange(MOE_INIT_ROUTING_V3_OUTPUT_EXPERT_TOKEN_CUMSUM_OR_COUNT);
    // OPS_LOG_E_IF_NULL(context, count, return ge::GRAPH_FAILED);
    auto expanded_scale = context->GetOutputShapeRange(MOE_INIT_ROUTING_V3_OUTPUT_EXPANDED_SCALE);
    // OPS_LOG_E_IF_NULL(context, expanded_scale, return ge::GRAPH_FAILED);

    // Print the shape ranges of the outputs before InferShapeRange
    // OPS_LOG_D(context, "Before InferShapeRange, expanded_x->GetMin() = %s",
    //         ToString(*(expanded_x->GetMin())).c_str());
    // OPS_LOG_D(context, "Before InferShapeRange, expanded_x->GetMax() = %s",
    //         ToString(*(expanded_x->GetMax())).c_str());

    // OPS_LOG_D(context, "Before InferShapeRange, expanded_row_idx->GetMin() = %s",
    //         ToString(*(expanded_row_idx->GetMin())).c_str());
    // OPS_LOG_D(context, "Before InferShapeRange, expanded_row_idx->GetMax() = %s",
    //         ToString(*(expanded_row_idx->GetMax())).c_str());

    // OPS_LOG_D(context, "Before InferShapeRange, count->GetMin() = %s",
    //         ToString(*(count->GetMin())).c_str());
    // OPS_LOG_D(context, "Before InferShapeRange, count->GetMax() = %s",
    //         ToString(*(count->GetMax())).c_str());

    // OPS_LOG_D(context, "Before InferShapeRange, expanded_scale->GetMin() = %s",
    //         ToString(*(expanded_scale->GetMin())).c_str());
    // OPS_LOG_D(context, "Before InferShapeRange, expanded_scale->GetMax() = %s",
    //         ToString(*(expanded_scale->GetMax())).c_str());
    ...
    // OPS_LOG_D(context, "After InferShapeRange, expanded_x->GetMin() = %s",
    //         ToString(*(expanded_x->GetMin())).c_str());
    // OPS_LOG_D(context, "After InferShapeRange, expanded_x->GetMax() = %s",
    //         ToString(*(expanded_x->GetMax())).c_str());

    // OPS_LOG_D(context, "After InferShapeRange, expanded_row_idx->GetMin() = %s",
    //         ToString(*(expanded_row_idx->GetMin())).c_str());
    // OPS_LOG_D(context, "After InferShapeRange, expanded_row_idx->GetMax() = %s",
    //         ToString(*(expanded_row_idx->GetMax())).c_str());

    // OPS_LOG_D(context, "After InferShapeRange, count->GetMin() = %s",
    //         ToString(*(count->GetMin())).c_str());
    // OPS_LOG_D(context, "After InferShapeRange, count->GetMax() = %s",
    //         ToString(*(count->GetMax())).c_str());

    // OPS_LOG_D(context, "After InferShapeRange, expanded_scale->GetMin() = %s",
    //         ToString(*(expanded_scale->GetMin())).c_str());
    // OPS_LOG_D(context, "After InferShapeRange, expanded_scale->GetMax() = %s",
    //         ToString(*(expanded_scale->GetMax())).c_str());

    // OPS_LOG_D(context, "End to do MoeInitRoutingV3InferRange.");
    return ge::GRAPH_SUCCESS;
}
