...
function help_info() {
    ...
    if [[ -n "$specific_help" ]]; then
        case "$specific_help" in
            package)
                ...
                echo "    --output-dir=<PATH>    Specify output directory, default: ./output"
                echo "    --build-out-dir=<PATH> Specify build_out directory, default: ./build_out"
    ...
    echo "    --build-dir=<PATH>     Specify build directory, default: ./build (autogen under <PATH>/autogen)"
    echo "    --output-dir=<PATH>    Specify output directory, default: ./output"
    echo "    --build-out-dir=<PATH> Specify build_out directory, default: ./build_out"
                echo $dotted_line
                return
    ...
    echo "build script for ops-xllm repository"
    echo "Usage:"
    ...
    echo "    --opgraph build graph_plugin_xllm.so"
    echo "    --onnxplugin build op_xllm_onnx_plugin.so"
    echo "    --opapi build opapi_xllm.so"
    echo "    --ophost build ophost_xllm.so"
    echo "    --opkernel build binary kernel"
    echo "    --jit build run package without kernel bin"
    echo "    --pkg build run package with kernel bin"
    echo "    --experimental build experimental version"
...
function clean()
{
    ...
    if [ -z "${TEST}" ] && [ -z "${EXAMPLE}" ];then
        if [ -n "${OUTPUT_DIR}" ];then
            if [[ "${OUTPUT_DIR}" == *"output"* || "${OUTPUT_DIR}" == *"xllm"* || "${OUTPUT_DIR}" == "/tmp/"* ]];then
                rm -rf ${OUTPUT_DIR}
            else
                log "Warning: OUTPUT_DIR=${OUTPUT_DIR} seems unusual, skipping rm -rf for safety"
            fi
        fi
    fi
...
function build_example()
{
    ...
    if [[ "${EXAMPLE_MODE}" == "eager" ]]; then
        ...
        for file in "${files[@]}"; do
            ...
            if [[ "${PKG_MODE}" == "" ]]; then
                g++ ${file} -I ${INCLUDE_PATH} -I ${ACLNN_INCLUDE_PATH} -I ${EAGER_INCLUDE_OPP_ACLNNOP_PATH} -L ${EAGER_LIBRARY_OPP_PATH} -L ${EAGER_LIBRARY_PATH} -lopapi_math -lopapi_xllm -lascendcl -lnnopbase -lpthread -lhccl -lhccl_fwk -lc_sec -o test_aclnn_${EXAMPLE_NAME}
            elif [[ "${PKG_MODE}" == "cust" ]]; then
                if [[ "${vendor_name}" == "" ]]; then
                    vendor_name="custom"
                fi
                echo "pkg_mode:${PKG_MODE} vendor_name:${vendor_name}"
                export CUST_LIBRARY_PATH="${ASCEND_OPP_PATH}/vendors/${vendor_name}_xllm/op_api/lib"     # 仅自定义算子需要
                export CUST_INCLUDE_PATH="${ASCEND_OPP_PATH}/vendors/${vendor_name}_xllm/op_api/include" # 仅自定义算子需要
                if [[ -n "${ASCEND_CUSTOM_OPP_PATH}" ]]; then
                    CUST_VENDORS_PATH=$(dirname "${ASCEND_CUSTOM_OPP_PATH%%:*}")
                    CUST_LIBRARY_PATH="${CUST_VENDORS_PATH}/${vendor_name}_xllm/op_api/lib"
                    CUST_INCLUDE_PATH="${CUST_VENDORS_PATH}/${vendor_name}_xllm/op_api/include"
                fi
...
package_static() {
    ...
    if [[ "$run_file" != *"ops-xllm"* ]]; then
        echo "Error: Filename '$run_file' does not contain 'ops-xllm'."
        return 1
    fi
    local static_name="${run_file/ops-xllm/ops-xllm-static}"
...
while [[ $# -gt 0 ]]; do
    ...
    --vendor_name=*)
        ...
        ;;
    --opgraph)
        BUILD_LIBS+=("opgraph_xllm")
        ENABLE_CREATE_LIB=TRUE
        OP_GRAPH=TRUE
        shift
        ;;
    --onnxplugin)
        BUILD_LIBS+=("op_xllm_onnx_plugin")
        ENABLE_CREATE_LIB=TRUE
        ONNX_PLUGIN=TRUE
        shift
        ;;
    --opapi)
        BUILD_LIBS+=("opapi_xllm")
        ENABLE_CREATE_LIB=TRUE
        OP_API=TRUE
        shift
        ;;
    --ophost)
        BUILD_LIBS+=("ophost_xllm")
        ENABLE_CREATE_LIB=TRUE
        OP_HOST=TRUE
        shift
        ;;
...
# 参数解析后同步 BUILD_PATH 与 BUILD_DIR
export BUILD_PATH="${BUILD_DIR}"
set_ut_mode
