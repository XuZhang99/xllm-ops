...
public:
    explicit CustomPagedAttentionOperation(const std::string &opName) noexcept : OperationBase(opName) {}
    ...
    Kernel *GetBestKernel(const LaunchParam &launchParam) const override
    {
        ...
        switch (param.type) {
            case OpParam::PagedAttention::PAGED_ATTENTION_MASK_ND:
                ...
                return GetKernelByName("CustomPagedAttentionMaskNdKernel");
...
private:
    ...
    bool CheckMask(const LaunchParam &launchParam, Tensor q, Tensor mask, bool nzFlag = false) const
    {
        ...
        if ((param.type == OpParam::PagedAttention::PAGED_ATTENTION_NZ_MASK) &&
            ...
            ATB_LOG(INFO) << "minKvSeqlenIter: " << *minKvSeqlenIter << " maxKvSeqlenIter: " << *maxKvSeqlenIter;
...
REG_OPERATION(CustomPagedAttentionOperation);
} // namespace Custom
} // namespace AtbOps
