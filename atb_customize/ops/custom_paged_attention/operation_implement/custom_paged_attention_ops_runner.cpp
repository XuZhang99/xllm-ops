...
std::size_t CustomPagedAttentionOpsRunner::IntensorSizeGenerate()
{
    // 6 after add custom_tiling_para_gm
    std::size_t intensorSize = 6;
...
CustomPagedAttentionOpsRunner::CustomPagedAttentionOpsRunner(const infer::PagedAttentionParam &param)
    : OpsRunner("CustomPagedAttentionOpsRunner"), param_(param)
{
    needKernelGraphModify_ = true;
    skipSetUpKernelGraphWhenCacheHit_ = false;
    ATB_LOG(INFO) << "CustomPagedAttentionOpsRunner::CustomPagedAttentionOpsRunner called";
    ...
    // ATB_LOG(INFO) << "query: " << ::atb::TensorUtil::AsdOpsTensorDescToString(query.desc);
    Mki::Tensor &keyCache = kernelGraph_.inTensors.at(inTensorStart++);
    // ATB_LOG(INFO) << "keyCache: " << ::atb::TensorUtil::AsdOpsTensorDescToString(keyCache.desc);
    Mki::Tensor *valueCache = isMla_ ? &nullTensor_ : &kernelGraph_.inTensors.at(inTensorStart++);
    // ATB_LOG(INFO) << "valueCache: " << ::atb::TensorUtil::AsdOpsTensorDescToString(valueCache->desc);
    Mki::Tensor &blockTables = kernelGraph_.inTensors.at(inTensorStart++);
    // ATB_LOG(INFO) << "blockTables: " << ::atb::TensorUtil::AsdOpsTensorDescToString(blockTables.desc);
    Mki::Tensor &contextLens = kernelGraph_.inTensors.at(inTensorStart++);
    // ATB_LOG(INFO) << "contextLens: " << ::atb::TensorUtil::AsdOpsTensorDescToString(contextLens.desc);
    (void)contextLens;
    ATB_LOG(INFO) << "infer::PagedAttentionParam.maskType: " << param.maskType;
    Mki::Tensor *mask = param_.maskType == atb::infer::PagedAttentionParam::MaskType::UNDEFINED ?
                            &nullTensor_ :
                            &kernelGraph_.inTensors.at(inTensorStart++);
    Mki::Tensor &custom_tiling_para_gm = kernelGraph_.inTensors.at(inTensorStart++);
    // ATB_LOG(INFO) << "custom_tiling_para_gm: " << ::atb::TensorUtil::AsdOpsTensorDescToString(custom_tiling_para_gm.desc);
    if (param_.batchRunStatusEnable) {
        inTensorStart++;
    }
    bool needQLens = (param_.calcType == atb::infer::PagedAttentionParam::CALC_TYPE_SPEC);
    ...
    pagedAttentionNode.opDesc = {0, "CustomPagedAttentionOperation", inPagedAttention};

    if (!isMla_) {
        pagedAttentionNode.inTensors = {&query,  &keyCache, valueCache, &blockTables, mask,  &custom_tiling_para_gm, kDescale,
...
CustomPagedAttentionOpsRunner::~CustomPagedAttentionOpsRunner() {}

AtbOps::OpParam::PagedAttention::MaskType CustomPagedAttentionOpsRunner::GetMaskType() const
{
    return static_cast<AtbOps::OpParam::PagedAttention::MaskType>(param_.maskType);
}

Status CustomPagedAttentionOpsRunner::ModifyKernelGraph(const OpsTensorPack &opsTensorPack)
{
    size_t batchRunStatusIndex = 6; // 5: batchRunStatus 位置
    if (param_.maskType != atb::infer::PagedAttentionParam::MaskType::UNDEFINED) {
        batchRunStatusIndex++;
    }
    size_t qLensIndex = 6; // 5: batchRunStatus 位置
    if (param_.maskType != atb::infer::PagedAttentionParam::MaskType::UNDEFINED) {
        qLensIndex++;
    }
    if (param_.batchRunStatusEnable) {
...
void CustomPagedAttentionOpsRunner::SetPaParam(AtbOps::OpParam::PagedAttention &inPagedAttention)
{
    if (isMla_) {
