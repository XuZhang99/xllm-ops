...
public:
    ...
    std::shared_ptr<TilingCases> RegisterOp(const std::string& op_type, int32_t arch)
    {
        ...
        if (soc_iter == registry_map_.end()) {
            ...
            registry_map_[arch] = op_type_map;
        } else {
        ...
        OPS_CHECK(registry_map_[arch][op_type] == nullptr,
    ...
    {
        int32_t arch = (int32_t)0xFFFF;
        const char* op_type = context->GetNodeType();
        fe::PlatFormInfos* platformInfoPtr = context->GetPlatformInfo();
        if (platformInfoPtr == nullptr) {
            OPS_LOG_E(op_type, "Do op tiling failed, cannot get platformInfo.");
            return ge::GRAPH_FAILED;
        } else {
            auto ascendcPlatform = platform_ascendc::PlatformAscendC(platformInfoPtr);
            arch = static_cast<int32_t>(ascendcPlatform.GetSocVersion());
            OPS_LOG_D(context, "npu arch is %d", arch);
            if (arch == (int32_t)0xFFFF) {
                OPS_LOG_E(op_type, "Do op tiling failed, cannot find npu arch.");
                return ge::GRAPH_FAILED;
            }
        }
        auto tilingTemplateRegistryMap = GetTilingTemplates(op_type, arch);
    ...
    {
        auto soc_iter = registry_map_.find(arch);
        OPS_CHECK(soc_iter == registry_map_.end(),
            OPS_LOG_E(op_type, "Get op tiling func failed, please check the npu arch %d", arch),
...
private:
    std::map<int32_t, std::map<std::string, std::shared_ptr<TilingCases>>> registry_map_; // key is npu-arch
